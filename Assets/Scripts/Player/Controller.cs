...
namespace Player
{
    public class Controller : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            animatorController = new AnimatorController(GetComponent<Animator>(), rb);
        }
        ...
        private void MoveCharacter(float horizontalMovement)
        {
            if (IsGrounded())
            {
                ...
                if (Mathf.Abs(rb.velocity.x) > Constants.MaxSpeedFactor)
                    rb.velocity = new Vector2(Mathf.Sign(rb.velocity.x) * Constants.MaxSpeedFactor,
                        rb.velocity.y);
            }
        }
        ...
        private void Jump()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space) && IsGrounded())
                rb.AddForce(
                    new Vector2(
                        isMoving
                            ? Mathf.Sign(horizontalMovement) * Constants.JumpHorizontalForce
                            : Constants.ZeroFloat,
